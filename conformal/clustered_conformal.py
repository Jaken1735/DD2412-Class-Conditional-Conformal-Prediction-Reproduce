...
def get_exact_coverage_conformal_params(scores, alpha):
    """
    Computes parameters q_a, q_b, and gamma to achieve exact coverage via randomized conformal prediction.
    ...
    Outputs:
        - q_a: Lower quantile.
        - q_b: Higher quantile.
        - gamma: Probability of using q_a.
    """
    n = len(scores)
    # Sort the scores
    sorted_scores = np.sort(scores)
    ...
    if k >= 1:
        ...
        q_a = -np.inf  # No lower quantile
    if k < n:
        q_b = sorted_scores[k]
    else:
        q_b = np.inf  # No higher quantile
    return q_a, q_b, gamma


def compute_exact_coverage_class_specific_params(scores, clusters, num_classes, alpha, default_qhat=np.inf, null_params=None):
    """
    Computes exact coverage parameters q_a, q_b, and gamma for each class or cluster.
    ...
    Inputs:
        ...
        - null_params: Dictionary containing 'q_a', 'q_b', and 'gamma' for clusters with no data.

    Outputs:
        - q_as: NumPy array of q_a values per cluster.
        - q_bs: NumPy array of q_b values per cluster.
        - gammas: NumPy array of gamma values per cluster.
    """
    # Initialize arrays
    q_as = np.full(num_classes, default_qhat)
    q_bs = np.full(num_classes, default_qhat)
    ...
    for c in range(num_classes):
        ...
        if n == 0:
            ...
            if null_params is not None:
                q_as[c] = null_params['q_a']
                q_bs[c] = null_params['q_b']
                gammas[c] = null_params['gamma']
            else:
                q_as[c] = default_qhat
                q_bs[c] = default_qhat
                gammas[c] = 1.0
            continue
        # Compute q_a, q_b, gamma for cluster c
